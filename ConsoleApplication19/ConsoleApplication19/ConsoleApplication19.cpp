...
void Calculatesallary(vector<Person*>& v) {
    ...
    cout << "Owner average percentage is " << summ3 / counto << endl << endl;
...
vector<string> Arrayofdata(vector<Person*>& l)
{
                ...
                for (long i = 0; i < len; i++) {
                 ...
                 //     std::cout << std::endl;
                    ...
                    if (persons.at(i)->category() == "Laborer") {
                        ...
                        outfile << persons.at(i)->category() << endl;
                        //  outfile << persons.at(i)->category();
       ...
       //         cout << text[i] << endl;
        //    }
            ...
            for (int i = 0; i < text.size(); i++) {
                if (text[i] == "Laborer") {
                    Laborer* lab = new Laborer();
                    lab->setName(text[i - 7]);
                    lab->setSsn(text[i - 6]);
                    lab->setBirthDate(text[i - 5]);
                    lab->setJob(text[i  - 4]);
                    lab->setEmpId(text[i  - 3]);
                    lab->setHourlySalary(stoi(text[i  - 2]));
                    lab->setHoursWorked(stoi(text[i  - 1]));
                    persons.push_back(lab);
                }

                else if (text[i] == "Manager") {
                    Manager* man = new Manager();
                    man->setName(text[i - 6]);
                    man->setSsn(text[i - 5]);
                    man->setBirthDate(text[i - 4]);
                    man->setDepartment(text[i - 3]);
                    man->setEmpId(text[i - 2]);
                    man->setSalary(stoi(text[i - 1]));
                    persons.push_back(man);

                }
                else if (text[i] == "Owner") {
                    Owner* owner = new Owner();
                    owner->setName(text[i - 5]);
                    owner->setSsn(text[i - 4]);
                    owner->setBirthDate(text[i - 3]);
                    owner->setBusinessPercentage(stoi(text[i - 2]));
                    owner->setOwnerDate(text[i - 1]);
                    persons.push_back(owner);
